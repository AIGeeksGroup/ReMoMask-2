...
def load(filename, start=None, end=None, order=None, world=False, need_quater=True):
    ...
    for line in f:
        ...
        if '}' in line:
            ...
            continue
        offmatch = re.match('\\s*OFFSET\\s+([\\-\\d\\.e]+)\\s+([\\-\\d\\.e]+)\\s+([\\-\\d\\.e]+)', line)
        if offmatch:
            if not end_site:
                offsets[active] = np.array([list(map(float, offmatch.groups()))])
            continue
        chanmatch = re.match('\\s*CHANNELS\\s+(\\d+)', line)
        if chanmatch:
    ...
    with open(filename, 'w') as f:
        ...
        for i in range(anim.shape[0]):
            for j in range(anim.shape[1]):
                if positions or j == 0:
                    f.write('%f %f %f %f %f %f ' % (poss[i, j, 0], poss[i, j, 1], poss[i, j, 2], rots[i, j, ordermap[order[0]]], rots[i, j, ordermap[order[1]]], rots[i, j, ordermap[order[2]]]))
                elif mask == None or mask[j] == 1:
                    f.write('%f %f %f ' % (rots[i, j, ordermap[order[0]]], rots[i, j, ordermap[order[1]]], rots[i, j, ordermap[order[2]]]))
                else:
                    f.write('%f %f %f ' % (0, 0, 0))
            f.write('\n')
