...
def remove_fs(glb, foot_contact, fid_l=(3, 4), fid_r=(7, 8), interp_length=5, force_on_floor=True):
    ...
    foot_heights = np.minimum(glb[:, fid_l, 1], glb[:, fid_r, 1]).min(axis=1)
    ...
    for (i, fidx) in enumerate(fid):
        ...
        for s in range(T):
            ...
            if not consl and (not consr):
                continue
            if consl and consr:
                litp = lerp(alpha(1.0 * (s - l + 1) / (interp_length + 1)), glb[s, fidx], glb[l, fidx])
                ritp = lerp(alpha(1.0 * (r - s + 1) / (interp_length + 1)), glb[s, fidx], glb[r, fidx])
