"""
Train QueryProjector: alignment from BMM teacher to z_e student.

The projector maps CLIP 512-d text embeddings into the VQ-VAE z_e 1024-d latent
space.  Three training objectives are supported via --objective (G6):
    kl       (default) KL(teacher||student) soft-target distillation: a frozen
             BMM (Part_TMR) retriever provides soft-target rankings over its
             top-K candidates, and the projector reproduces those rankings in
             z_e space. This is the original behaviour.
    infonce  Single-positive in-batch InfoNCE baseline: for each caption the
             positive is its own paired z_e database entry, negatives are the
             other in-batch samples' paired entries. Contrasts the soft-target
             KL objective against a hard-label contrastive baseline.
    mse      Cosine regression to the paired z_e key: loss = mean(1 - cos(.)).
             Since both vectors are L2-normalised this equals 0.5*MSE up to a
             constant; implemented directly as 1-cos for clarity/stability.

Capacity is configurable via --hidden (G7): 0 = single Linear(clip_dim->ze_dim),
1024 (default) = current 2-layer MLP, 2048 = wider 2-layer MLP.
...
def train(args):
    ...
    #    Only needed for --objective kl; infonce/mse use the paired z_e
    #    entry directly and skip this precomputation entirely.
    # ------------------------------------------------------------------
    K_teacher = None
    ...
    if args.objective == "kl":
        ...
        print(f"  Objective={args.objective}: skipping BMM teacher precomputation "
              f"(paired z_e entries are used directly).")

    # ------------------------------------------------------------------
    # 3. Build model, optimiser, scheduler
    # ------------------------------------------------------------------
    projector = QueryProjector(clip_dim=clip_dim, ze_dim=ze_dim, hidden=args.hidden).to(device)
    n_params = sum(p.numel() for p in projector.parameters())
    print(f"QueryProjector: {n_params:,} params  "
          f"(clip_dim={clip_dim}, ze_dim={ze_dim}, hidden={args.hidden}, objective={args.objective})")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, N, args.batch_size):
            ...
            projected = projector(batch_clip)                # (B, ze_dim), L2-normalised
            ...
            if args.objective == "kl":
                ...
                # Single-positive in-batch InfoNCE baseline (contrasted with the
                # soft-target KL objective above). For each caption in the batch,
                # the positive is its own paired z_e database entry (same index);
                # negatives are the other in-batch samples' paired entries.
                target_ze = ze_motions[batch_idx]                # (B, ze_dim)
                logits = (projected @ target_ze.T) / temperature  # (B, B)
                labels = torch.arange(B, device=device)
                loss = F.cross_entropy(logits, labels)

            elif args.objective == "mse":
                # Cosine regression to the paired z_e key: loss = mean(1 - cos(.)).
                # projector() output and ze_motions are both L2-normalised, so the
                # dot product below is exactly the cosine similarity, and
                # 1 - cos(a, b) == 0.5 * ||a - b||^2 for unit vectors (i.e. this is
                # 0.5*MSE up to the constant factor); implemented as 1-cos directly.
                target_ze = ze_motions[batch_idx]                # (B, ze_dim)
                cos_sim = (projected * target_ze).sum(dim=1)     # (B,)
    ...
    save_projector(projector, last_path, extra={
        "epoch": args.epochs, "loss": avg_loss, "objective": args.objective,
    })
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train QueryProjector: BMM teacher -> z_e student alignment "
                    "(--objective kl/infonce/mse, --hidden 0/1024/2048)")
    ...
    parser.add_argument("--teacher_topk", type=int, default=256,
                        help="Number of BMM teacher candidates per text (efficiency); "
                             "only used by --objective kl")
    parser.add_argument("--objective", type=str, default="kl",
                        choices=["kl", "infonce", "mse"],
                        help="Training objective (G6): kl = KL(teacher||student) soft-target "
                             "distillation over BMM top-K candidates (default, original behaviour); "
                             "infonce = single-positive in-batch InfoNCE against the paired z_e "
                             "entry, contrasted with soft-target kl; "
                             "mse = cosine regression (1-cos) to the paired z_e key")
    parser.add_argument("--hidden", type=int, default=1024, choices=[0, 1024, 2048],
                        help="QueryProjector capacity (G7): 0 = single Linear(clip_dim->ze_dim); "
