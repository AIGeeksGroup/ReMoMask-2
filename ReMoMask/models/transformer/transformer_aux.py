...
class MaskTransformer(nn.Module):
    ...
    def trans_forward(self, motion_ids, cond, padding_mask, force_mask=False):
        ...
        cond = cond.unsqueeze(0)   #(1, b, latent_dim)  # (1, 64, 512)
...
class ResidualTransformer(nn.Module):
    ...
    def trans_forward(self, motion_codes, qids, cond, padding_mask, force_mask=False):
        ...
        # text projection
        cond = self.cond_emb(cond)  # (b, latent_dim)  # (512 -> 384)
        cond = cond.unsqueeze(0)   #(1, b, latent_dim)  # (1, 64, 512)
    ...
    def forward(self, all_indices, y, m_lens):
        ...
        :param y: prompt embedding  # (b, 512)
        :m_lens: (b,)
        :return:
        '''
    ...
    def generate(self,
        ...
        cond_vector = conds.to(device).float()  # prompt text embedding:  (1, b, d)   (1, 32, 512)
        # import ipdb; ipdb.set_trace()
        # with torch.no_grad():
        #     cond_vector = self.encode_text(conds)  # prompt text embedding:  (1, b, d)   (1, 32, 384)
        # if self.cond_mode == 'text':
        #     with torch.no_grad():
        #         cond_vector = self.encode_text(conds)
        # elif self.cond_mode == 'action':
