...
if __name__ == '__main__':
    ...
    if getattr(opt, 'use_ze_retrieval', False):
        ...
        # G4: retrieval_topk/retrieval_pool override ZeRetriever's ctor defaults (top_k/num_retrieval)
        ze_kwargs = {}
        if opt.retrieval_topk is not None:
            ze_kwargs['top_k'] = opt.retrieval_topk
        if opt.retrieval_pool is not None:
            ze_kwargs['num_retrieval'] = opt.retrieval_pool
        ...
        print(f"[V2] ZeRetriever loaded (database={opt.ze_database_path}, "
              f"top_k={retriever.top_k}, num_retrieval={retriever.num_retrieval})")
    else:
        from hydra import initialize, compose
        with initialize(config_path="Part_TMR/conf", version_base=None):
            rag_cfg = compose(config_name="config")
        # G4: retrieval_topk/retrieval_pool override cfg.rag.top_k/cfg.rag.num_retrieval
        if opt.retrieval_topk is not None:
            rag_cfg.rag.top_k = opt.retrieval_topk
        if opt.retrieval_pool is not None:
            rag_cfg.rag.num_retrieval = opt.retrieval_pool
        retriever = load_retr_model(rag_cfg)
        retriever.to(opt.device)
        retriever.eval()
        print(f"[V1] MocoTmrRetriever loaded (top_k={retriever.top_k}, num_retrieval={retriever.num_retrieval})")

    # G4: guard — top_k must not exceed the candidate pool, after default resolution
    if retriever.top_k > retriever.num_retrieval:
        raise ValueError(
            f"--retrieval_topk ({retriever.top_k}) must be <= --retrieval_pool ({retriever.num_retrieval})"
        )
