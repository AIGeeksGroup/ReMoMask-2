...
if __name__ == '__main__':
    ...
    caption_ids = retriever.tokenize(captions).to(opt.device)  # (b, ntokens)
    ...
    for r in range(opt.repeat_times):
        print("-->Repeat %d"%r)
        with torch.no_grad():
            mids_aux = mask_transformer_aux.generate(caption_embedding, token_lens,
                                            timesteps=opt.time_steps,
                                            cond_scale=opt.cond_scale,
                                            temperature=opt.temperature,
                                            topk_filter_thres=opt.topkr,
                                            gsample=opt.gumbel_sample)
            mids_ts = mask_transformer_ts.generate(caption_embedding, token_lens,
                                            timesteps=opt.time_steps,
                                            cond_scale=opt.cond_scale,
                                            temperature=opt.temperature,
                                            topk_filter_thres=opt.topkr,
                                            gsample=opt.gumbel_sample, n_j=J, re_dict=re_dict)
            mids_aux = res_transformer_aux.generate(mids_aux, caption_embedding, token_lens, temperature=1, cond_scale=5)
            mids_ts = res_transformer_ts.generate(mids_ts, caption_embedding, token_lens, temperature=1, cond_scale=5)
