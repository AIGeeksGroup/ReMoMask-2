...
def plot_t2m(data, save_dir):
    data = train_dataset.inv_transform(data)#####[6,196,263]
    # import ipdb;ipdb.set_trace()
    for i in range(len(data)):
        joint_data = data[i]####[196,263]
        # import ipdb;ipdb.set_trace()
        joint = recover_from_ric(torch.from_numpy(joint_data).float(), opt.joints_num).numpy()###[196,22,3]--->bvh
        # import ipdb;ipdb.set_trace()
        save_path = pjoin(save_dir, '%02d.mp4' % (i))
        plot_3d_motion(save_path, kinematic_chain, joint, title="None", fps=fps, radius=radius)####visualization
    # import ipdb;ipdb.set_trace()
