...
class TextMotionDataset(data.Dataset):
    ...
    def __init__(self, cfg, mean, std, split_file, eval_mode=False, fps=None):
        self.cfg = cfg
                ...
                with cs.open(pjoin(cfg.dataset.text_dir, name + '.txt')) as f:
                    for (index, line) in enumerate(f.readlines()):
                        ...
                        if f_tag == 0.0 and to_tag == 0.0:
                            ...
                            n_motion = motion[int(f_tag * cfg.dataset.fps):int(to_tag * cfg.dataset.fps)]
    ...
    def __getitem__(self, item):
        ...
        if m_length >= self.max_motion_length:
            idx = random.randint(0, len(motion) - max_motion_length) if not self.eval_mode else 0
            motion = motion[idx:idx + max_motion_length]
            m_length = max_motion_length
        elif self.cfg.preprocess.padding:
        ...
        (Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm) = whole2parts(motion, mode='t2m' if self.cfg.dataset.dataset_name == 'HumanML3D' else 'kit')
