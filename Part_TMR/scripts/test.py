...
def prepare_test_dataset(cfg):
    ...
    if cfg.eval.eval_train:
        test_split_file = pjoin(cfg.dataset.data_root, cfg.dataset.train_split_filename)
    else:
        test_split_file = pjoin(cfg.dataset.data_root, 'test.txt')
    test_dataset = TextMotionDataset(cfg, mean, std, test_split_file, eval_mode=True, fps=True)
    test_dataloader = DataLoader(test_dataset, batch_size=cfg.train.batch_size, shuffle=False, num_workers=16)
...
def prepare_test_model(cfg):
    ...
    model = MoCoTMR(text_encoder_alias, text_encoder_trainable, motion_embedding_dims, text_embedding_dims, projection_dims, mode='t2m' if cfg.dataset.dataset_name == 'HumanML3D' else 'kit', config=cfg)
...
def eval(cfg, test_dataloader, model, verbose=True):
    ...
    with torch.no_grad():
        ...
        for batch in test_pbar:
            step += 1
            (Root, R_Leg, L_Leg, Backbone, R_Arm, L_Arm, texts, m_length, img_indexs) = batch
            ...
            motion_dict = model.encode_motion(motions)
            motion_features = motion_dict['global']
