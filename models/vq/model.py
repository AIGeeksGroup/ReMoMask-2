...
class RVQVAE(nn.Module):
    ...
    def __init__(self, args, input_width=263, down_t=3, stride_t=2, width=512, depth=3, dilation_growth_rate=3, activation='relu', norm=None):
        super().__init__()
        self.code_dim1d = args.code_dim1d
        self.num_code1d = args.nb_code1d
        self.encoder1d = Encoder(input_width, self.code_dim1d, down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
        self.decoder1d = Decoder(input_width, self.code_dim1d, down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
        rvqvae_config = {'num_quantizers': args.num_quantizers, 'shared_codebook': args.shared_codebook, 'quantize_dropout_prob': args.quantize_dropout_prob, 'quantize_dropout_cutoff_index': 0, 'nb_code': self.num_code1d, 'code_dim': self.code_dim1d, 'args': args}
        ...
        self.encoder2d = Encoder2d(self.encode_dim2d, self.code_dim2d, down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
        self.linear_merge = nn.Linear(self.code_dim2d * self.J, self.code_dim2d * self.J)
        self.decoder2d = Decoder2d(self.encode_dim2d, self.code_dim2d, down_t, stride_t, width, depth, dilation_growth_rate, activation=activation, norm=norm)
        self.linear_out = nn.Linear(input_width * 2, input_width)
        joints_rvqvae_config = {'num_quantizers': args.num_quantizers, 'shared_codebook': args.shared_codebook, 'quantize_dropout_prob': args.quantize_dropout_prob, 'quantize_dropout_cutoff_index': 0, 'nb_code': self.num_code2d, 'code_dim': self.code_dim2d, 'args': args}
    ...
    def encode(self, x1d, x2d):
        ...
        (B, T0, _, _) = x2d.shape
        if self.J0 == 22:
            x2d = torch.nn.functional.pad(x2d, (0, 0, 1, 1))
        elif self.J0 == 21:
            x2d = torch.nn.functional.pad(x2d, (0, 0, 1, 2))
        x2d_encode = self.encoder2d(rearrange(x2d, 'b t j d -> b d t j'))
        (B, D, T, _) = x2d_encode.shape
        (code_idx2d, all_codes2d) = self.quantizer2d.quantize(rearrange(x2d_encode, 'b d t j -> (b j) d t'))
    ...
    def forward_decoder(self, x1d, x2d):
