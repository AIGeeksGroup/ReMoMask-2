...
class MotionDataset(data.Dataset):
    ...
    def __init__(self, opt, mean, std, split_file):
        ...
        if opt.is_train:
            ...
            std[4 + (joints_num - 1) * 9 + joints_num * 3:] = std[4 + (joints_num - 1) * 9 + joints_num * 3:] / opt.feat_bias
...
class Text2MotionDatasetEval(data.Dataset):
                ...
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    for line in f.readlines():
                        ...
                        if f_tag == 0.0 and to_tag == 0.0:
                            ...
                            try:
                                n_motion = motion[int(f_tag * (12.5 if opt.dataset_name == 'kit' else 20)):int(to_tag * (12.5 if opt.dataset_name == 'kit' else 20))]
    ...
    def __getitem__(self, item):
        ...
        if self.opt.dataset_name == 'humanml3d' or self.opt.dataset_name == 't2m':
            n_j = 22
        elif self.opt.dataset_name == 'kit':
            n_j = 21
        n_feat = 12
