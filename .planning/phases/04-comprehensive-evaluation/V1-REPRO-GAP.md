...
- **E12(对称重训 rtrans)正式取消(2026-07-06 深夜,用户驳斥后确认)**:①"全组件自训"声明空洞——pipeline 里 VQ-VAE 本就冻结用官方的(LOCKED-02),对称使用发布组件是标准做法;②"偏爱官方风格"疑虑早已被 teacher-forced-on-GT 论证排除,同配方重训消不掉精修不对称(那是两个 mtrans 输出分布的 finding);③成本不是收益。rtrans 相关唯一存活项 = 「检索条件化 rtrans」(架构改动 = 新贡献,scope 另议)。
...
**arXiv 主表行逐指标归属(2026-07-06 深夜,用户提供 arXiv 表触发;E0c 副产品验证)**:arXiv ReMoMask 行 = Top1/2/3 0.531/0.722/0.813、FID 0.099、MM-Dist 2.865、MMod 2.823。**表内自曝红旗:R-Precision 三项与 MM-Dist 全部优于同表 Real Motions 行**(0.531>0.511 等)——生成动作不可能系统性超过真实动作,GT-base 重建口径则天然如此。E0c 实测 GT-base 口径:R_pred 0.508-0.511/0.710-0.719/0.807-0.814(可反超 GT 侧)、matching ~2.90 —— **arXiv 行的 R-P/MM-Dist 与 GT-base 带吻合,与 T2M full 实测(0.485/0.676/0.777、3.090)不符**;FID 0.099 则与 mask-only/mtrans 内录 Value(0.0934)带吻合 → arXiv 行疑似**跨口径混排**;camera-ready 0.026 与 MMod 2.835 归 GT-base(已实测)。结论:**两版论文的 V1 行均无法以 released 资产在 T2M 口径复现**,统一重评协议是唯一自洽出路(§3.86)。published 0.026/0.566/2.835 与 released ckpt 在一致协议下的实测(0.123/0.485/1.446)系统性不符;0.026 恰落于该 codebase 三条 FID 输出路径中唯一匹配的 GT-base 宇宙(rtrans ckpt 内录 0.02211,一作训练日志 0.0221-0.0296);MMod 双机佐证(我们链 1.446,一作自己机器 1.332)排除环境解释。**加上 DIFF-AUDIT.md 的权重法证(pristine 代码无法加载官方 ckpt、"官方代码参照系"不存在、我们的链是唯一与 released 权重结构一致的链),H1 以现有证据判定成立;表述纪律照 §3.86(不指控,诚实口径差异框架)。**
